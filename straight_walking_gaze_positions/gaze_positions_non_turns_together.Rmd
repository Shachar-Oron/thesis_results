---
title: "gaze_positions_non_turns_together"
output: 
  pdf_document:
...
create_combined_graphs <- function(file) {
  ...
  # Identify segments based on time jumps
  df$segment <- cumsum(c(TRUE, diff(df[[time_col_name]]) > 5))
  
  # Normalize time within each segment
  df <- df %>%
    group_by(segment) %>%
  ...
  vertical_plot <- ggplot(df, aes(x = normalized_time, y = norm_pos_y, color = factor(segment))) +
    geom_line(linewidth = 0.5, na.rm = TRUE) +
    scale_color_discrete(name = "Segment") +
    scale_y_continuous(limits = c(-500, 1000), breaks = seq(-500, 1000, by = 250)) +
    ggtitle(paste("Vertical gaze positions - Straight Walking", patient_id)) +
  ...
  vertical_plot_file <- file.path(dirname(file), paste0("combined_vertical_gaze_straight_", patient_id, ".png"))
  ...
  horizontal_plot <- ggplot(df, aes(x = normalized_time, y = norm_pos_x, color = factor(segment))) +
    geom_line(linewidth = 0.5, na.rm = TRUE) +
    scale_color_discrete(name = "Segment") +
    scale_y_continuous(limits = c(-500, 1000), breaks = seq(-500, 1000, by = 250)) +
    ggtitle(paste("Horizontal gaze positions - Straight Walking", patient_id)) +
    xlab("Normalized Time") +
  ...
  horizontal_plot_file <- file.path(dirname(file), paste0("combined_horizontal_gaze_straight_", patient_id, ".png"))
  ggsave(horizontal_plot_file, horizontal_plot, width = 12, height = 6, dpi = 300)
  
  # Print the horizontal plot
  print(horizontal_plot)
  
  cat(paste("Combined graphs created for patient (straight walking):", patient_id, "\n"))
}

...
create_combined_graphs <- function(file) {
  ...
  vertical_plot <- ggplot(df, aes(x = !!sym(time_col_name), y = norm_pos_y, color = factor(segment))) +
  ...
  horizontal_plot <- ggplot(df, aes(x = !!sym(time_col_name), y = norm_pos_x, color = factor(segment))) +
    geom_line(linewidth = 0.5, na.rm = TRUE) +
    scale_color_discrete(name = "Segment") +
    scale_y_continuous(limits = c(-500, 1000), breaks = seq(-500, 1000, by = 250)) +
